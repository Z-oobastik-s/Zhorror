...
export class VoidScene extends Scene {
  ...
  protected build(): void {
    ...
    header.append(
      ...
      this.createEl('p', 'zh-void__hint', 'введи id записи-печати из архива'),
    );

    this.timerEl = this.createEl('div', 'zh-void__timer', '00:00');
    this.messageEl = this.createEl('p', 'zh-void__message',
      'Ты дошёл до края. Одна из записей архива - ключ. Открой запретный фрагмент.');
    ...
    if (quest.isAct1Complete()) this.showEnding();
  }

  private tryCode(): void {
    if (!quest.canInteract() || quest.isAct1Complete()) return;
