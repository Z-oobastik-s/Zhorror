...
import { CATACOMB_MARKS, SCENE_IDS } from '@/config/constants';
...
export class CatacombsScene extends Scene {
  ...
  protected build(): void {
    ...
    if (quest.getCatacombMarks().length >= CATACOMB_MARKS.length) {
      this.done = true;
  ...
  private onDoor(rune: string, door: HTMLElement): void {
    ...
    this.statusEl.textContent = `${quest.getCatacombMarks().length} / 4`;
    if (result === 'done') {
      this.done = true;
