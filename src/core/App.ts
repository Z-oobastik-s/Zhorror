...
export class App {
  ...
  async registerScenes(scenes: Scene[]): Promise<void> {
    ...
    for (const scene of scenes) {
      ...
      el.querySelectorAll('.zh-archive__card, .zh-hero__sigil, .zh-nav__sigil, .zh-ritual__symbol, .zh-void__submit').forEach((node) => {
  ...
  private bindGlobalEvents(): void {
    ...
    events.on(EVT.QUEST_CHAPTER, () => this.nav.refreshLocks());
